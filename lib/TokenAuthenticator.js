...
function refreshAccessToken (creds, token) {
	/*
	...
	*/
	...
	var authHeader = new Buffer(creds.clientId + ":" + creds.clientSecret).toString("base64");
	const headers = {
		"Content-Type": "application/x-www-form-urlencoded",
...
function getBearerToken (idBroker, credentials) {
	const body = {
		name: credentials.name,
		password: credentials.password
	}
	const headers = {"Content-Type": "application/json"}
	const url = `https://${idBroker}/idb/token/${credentials.orgId}/v2/actions/GetBearerToken/invoke`
	return request.post({
		url,
...
async function getAccessToken (connectionData, labMode, includeKmsScopes){
	try {
		// decode the connection data string into JSON
		const json = decodeConnectionData(connectionData)
		// console.log('decodeConnectionData', json)
		// get credentials for lab mode or production mode
		const credentials = labMode ? json.credentials : json.credentialsLabMode
		// console.log('credentials', credentials)
		// get bearer token
		const bearer = await getBearerToken(json.identityBrokerUrl, credentials)
		// console.log('bearer', bearer)
		const bearerToken = JSON.parse(bearer)["BearerToken"]
		// console.log('bearerToken', bearerToken)
		// create auth header with credentials
		const authHeader = new Buffer(credentials.clientId + ":" + credentials.clientSecret).toString("base64")
		const scopes = includeKmsScopes ? [SCOPE, KMS_SCOPES].join(" ") : SCOPE
		const form = {
		...
		const url = `https://${json.identityBrokerUrl}/idb/oauth2/v1/access_token`
		return request.post({
			url,
			body,
			headers
