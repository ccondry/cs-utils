...
async function refreshAccessToken (connectionDataString, labMode = true, accessToken) {
	...
	const connectionData = lib.utils.decodeConnectionData(connectionDataString)
	// get credentials
	const credentials = lib.utils.getCredentials(connectionData, labMode)
	// create auth header with credentials
	const authHeader = lib.utils.getAuthHeader(credentials)
	// create HTTP headers
	const headers = {
