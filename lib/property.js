		...
		return poll(function(){
			...
			return rest.getApi(getPath, access_token)
				.then(function(response){
					...
					return doFeaturesMatch(propertyValue, response.value);
				})
		}).then(function () {
...
/**
 * Checks if two comma-separated lists of features match.
 */
function doFeaturesMatch (featureList1, featureList2) {
	var parseFeatures = function (features) {
		return features.split(',').sort(); // We need to sort the lists because the order doesn't matter
	};

	return _.equals(parseFeatures(featureList1), parseFeatures(featureList2));
}
